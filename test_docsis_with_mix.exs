# Test all DOCSIS MIBs with the ported parser using mix run
...
defmodule ComprehensiveDocsisTest do
  ...
  def run_test do
    ...
    IO.puts("🔬 COMPREHENSIVE DOCSIS MIB TEST - GRAMMAR-BASED PARSER")
    IO.puts("=" |> String.duplicate(80))
  ...
  defp print_final_summary(results, total_count) do
    IO.puts("")
    IO.puts("=" |> String.duplicate(80))
    IO.puts("📈 FINAL COMPREHENSIVE RESULTS - GRAMMAR-BASED PARSER")
    IO.puts("=" |> String.duplicate(80))
    ...
    previous_success_rate = 3.6  # Previous result with original parser
    
    cond do
      success_rate >= 90 ->
        IO.puts("🌟 EXCELLENT: #{success_rate}% success - Grammar-based parser handles DOCSIS MIBs very well!")
      success_rate >= 75 ->
    ...
    # Show improvement vs previous parser
    if success_rate > previous_success_rate do
      improvement = success_rate - previous_success_rate
      IO.puts("📈 IMPROVEMENT: +#{Float.round(improvement, 1)}% vs previous parser (#{previous_success_rate}%)")
    elsif success_rate == previous_success_rate do
      IO.puts("📊 SAME: #{success_rate}% success rate - no improvement vs previous parser")
    else
      regression = previous_success_rate - success_rate  
      IO.puts("📉 REGRESSION: -#{Float.round(regression, 1)}% vs previous parser (#{previous_success_rate}%)")
    end
