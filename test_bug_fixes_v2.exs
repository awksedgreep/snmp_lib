...
defmodule BugFixTests do
  def test_object_identifier_string_validation do
    ...
    # This should now return an error instead of silently converting to null
    ...
    case SnmpLib.PDU.encode_message(message) do
      {:error, {:encoding_error, %ArgumentError{} = e}} ->
      ...
      {:error, other} ->
        IO.puts("❌ FAIL: Expected ArgumentError but got: #{inspect(other)}")
    end
    ...
    case SnmpLib.PDU.encode_message(message) do
      {:ok, encoded} ->
        case SnmpLib.PDU.decode_message(encoded) do
          ...
          {:error, decode_error} ->
            IO.puts("❌ FAIL: Decoding failed: #{inspect(decode_error)}")
        end
      {:error, encode_error} ->
        IO.puts("❌ FAIL: Encoding failed: #{inspect(encode_error)}")
    end
  ...
  def test_end_of_mib_view_tuple_validation do
    ...
    # This should now return an error instead of accepting the tuple
