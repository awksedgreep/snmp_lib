defmodule SnmpLib.MIB.Preprocessor do
  @moduledoc """
  ...
  defp simplify_enumeration_content(enumeration) do
    ...
    simplified_items = lines
    |> Enum.map(&extract_enum_item/1)
    |> Enum.filter(& &1)  # Remove nils
    |> Enum.take(10)      # Take only first 10 items to avoid parser issues
    |> Enum.with_index()
  ...
  def normalize_whitespace(content) do
    content
    |> String.replace(~r/[ \t]+/, " ")        # Replace multiple spaces/tabs with single space
  ...
  def analyze_enumerations(content) do
    ...
    |> Enum.map(fn [_full, enumeration] ->
      lines = String.split(enumeration, "\n")
      items = Enum.count(lines, fn line ->
