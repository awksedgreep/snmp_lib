defmodule SnmpLib.MIB.Utilities do
  @moduledoc """
  ...
  def check_sub_ids(oid, min_value, max_value) do
    case Enum.find(oid, &(&1 < min_value or &1 > max_value)) do
      nil -> :ok
      invalid_id -> {:error, "Sub-identifier #{invalid_id} out of range [#{min_value}..#{max_value}]"}
    end
  end
    ...
    case Map.get(entry, :oid) do
      ...
      symbolic_oid ->
        case tr_oid(symbolic_oid, oid_table) do
          {:ok, numeric_oid} ->
            vprint(verbosity, :debug, "Utilities", "update_oid", 
                   "Resolved ~s -> ~w", [symbolic_oid, numeric_oid])
  ...
  defp validate_integer_constraints([{:range, min, max} | rest]) when is_integer(min) and is_integer(max) and min <= max do
  ...
  defp validate_size_constraints([{:range, min, max} | rest]) when is_integer(min) and is_integer(max) and min <= max and min >= 0 do
    validate_size_constraints(rest)
  end
