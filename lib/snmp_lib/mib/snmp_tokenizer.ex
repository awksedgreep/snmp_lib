defmodule SnmpLib.MIB.SnmpTokenizer do
  @moduledoc """
  ...
  """
  def test() do
    test_string = ~c"TEST-MIB DEFINITIONS ::= BEGIN testObject OBJECT IDENTIFIER ::= { test 1 } END"
  ...
  defp tokenise(%__MODULE__{chars: [char | _chars]} = state) when (char >= ?a and char <= ?z) or (char >= ?A and char <= ?Z) or char == ?_ do
  ...
  defp scan_integer_digits([digit | chars], acc, line, state) when (digit >= ?0 and digit <= ?9) or (digit >= ?a and digit <= ?f) or (digit >= ?A and digit <= ?F) do
  ...
  defp scan_integer_digits(chars, acc, line, state) do
    ...
    has_hex_digits = Enum.any?(integer_chars, fn char -> 
  ...
  defp scan_name([char | chars], acc, line, state) when (char >= ?a and char <= ?z) or 
  ...
  defp scan_name(chars, acc, line, state) do
    ...
    token = case classify_name(name_string, name_chars) do
      {:reserved, atom} ->
        {atom, line}
      {:variable, _} ->
