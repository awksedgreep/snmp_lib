defmodule SnmpLib.RFCComplianceTest do
  ...
  describe "RFC 1157 (SNMPv1) Compliance" do
    test "validates SNMPv1 PDU type restrictions" do
      ...
      for pdu_type <- valid_v1_types do
        case pdu_type do
          :get_request ->
            pdu = PDU.build_get_request_multi([{[1, 3, 6, 1], :null, nil}], 1)
          ...
          # TRAP handled separately due to different structure
          _ ->
            :ok
        end
      end
    ...
    test "validates community string handling per RFC 1157" do
      ...
      assert :ok =
               PDU.validate_community(
                 elem(
                   PDU.encode_message(
                     PDU.build_message(
                       PDU.build_get_request_multi([{[1, 3, 6, 1], :null, nil}], 1),
                       "test_community"
                     )
                   ),
                   1
      ...
      assert {:error, :invalid_community} =
               PDU.validate_community(
                 elem(
                   PDU.encode_message(
                     PDU.build_message(
  ...
  describe "RFC 1905 (SNMPv2c) Protocol Operations" do
    ...
    test "validates mixed exception and normal values in responses" do
      ...
      [{_, type1, val1}, {_, type2, val2}, {_, type3, val3}, {_, type4, val4}] =
        decoded.pdu.varbinds
    ...
    test "rejects exception values in SNMPv1 context" do
      ...
      case result do
        # Expected - should reject exception values in v1
        {:error, _} ->
          :ok
  ...
  describe "ASN.1 BER Encoding Compliance (ITU-T X.690)" do
    test "validates length encoding forms per X.690" do
      ...
      long_lengths = [128, 200, 300, 1000, 65535, 1_000_000]

      for length <- long_lengths do
        encoded_length = ASN1.encode_length(length)
        {:ok, {decoded_length, <<>>}} = ASN1.decode_length(encoded_length)
        assert decoded_length == length
        # At least 2 bytes for long form
        assert byte_size(encoded_length) >= 2
      end
    end
    ...
    test "rejects indefinite length encoding (0x80)" do
      ...
      assert {:error, :indefinite_length_not_supported} =
               ASN1.decode_length(indefinite_length_data)
    end

    test "handles malformed length sequences" do
      # Test various malformed length encodings
      malformed_cases = [
        # Claims 5 length bytes but provides none
        <<0x85>>,
        # Claims 2 length bytes but provides only 1
        <<0x82, 0x01>>,
        # Claims 4 bytes but incomplete
        <<0x84, 0xFF, 0xFF, 0xFF>>,
        # Too many length bytes (>4)
    ...
    test "validates tag class and constructed bit handling" do
      ...
      tag_classes = [
        # INTEGER
        {0x02, :universal, false},
        # SEQUENCE (constructed)
        {0x30, :universal, true},
        # Application class, primitive
        {0x41, :application, false},
        # Application class, constructed
        {0x61, :application, true},
        # Context class, primitive
        {0x80, :context, false},
        # Context class, constructed
        {0xA0, :context, true},
  ...
  describe "OID Tree Structure RFC Compliance" do
    ...
    test "validates OID first arc restrictions per X.660" do
      ...
      valid_first_arcs = [
        # ccitt, max second arc
        [0, 39],
        # iso, max second arc
        [1, 39],
        # iso.identified-organization.dod.internet
        [1, 3, 6, 1],
        # joint-iso-ccitt, any second arc
        [2, 100, 3]
      ]
      ...
      invalid_first_arcs = [
        # Invalid first arc
        [3, 1],
        # Second arc too large for first arc 0
        [0, 40],
        # Second arc too large for first arc 1
        [1, 40]
      ]
    ...
    test "validates SNMP table index operations" do
      ...
      # ifDescr
      table_base = [1, 3, 6, 1, 2, 1, 2, 2, 1, 1]
  ...
  describe "Error Handling and Recovery" do
    test "handles malformed packet recovery" do
      ...
      malformed_packets = [
        ...
        # Invalid tags
        <<0xFF, 0xFF, 0xFF>>,
        # Extremely large claimed length
        String.duplicate(<<0x30, 0x82, 0x10, 0x00>>, 100)
      ]
    ...
    test "validates error-index correlation with varbinds" do
      ...
      # badValue error on varbind 2
      error_pdu = PDU.build_response(1, 3, 2, varbinds)
      ...
      # Points to second varbind
      assert decoded.pdu.error_index == 2
    ...
    test "handles resource exhaustion scenarios" do
      ...
      large_pdu = PDU.build_get_request_multi(large_varbinds, 1)
  ...
  describe "Performance and Concurrent Safety" do
    ...
    test "validates encoding/decoding performance bounds" do
      ...
      large_pdu = PDU.build_get_request_multi(large_varbinds, 2)
