defmodule SnmpLib.Integration.Phase2Test do
  ...
  describe "Full Stack Integration" do
    test "complete SNMP message workflow" do
      ...
      {:ok, request_pdu} = PDU.build_get_request_multi(varbinds, 12345)
    ...
    test "handles complex data types in full workflow" do
      ...
      typed_varbinds = Enum.map(varbinds, fn {oid, value} -> 
        {oid, :string, value} 
      end)
    ...
    test "error handling across modules" do
      ...
      {:ok, pdu} = PDU.build_get_request_multi(invalid_varbinds, 456)
